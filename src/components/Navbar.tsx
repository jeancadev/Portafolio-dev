      ...
      <nav className="container mx-auto px-4 sm:px-6 md:px-8 max-w-5xl">
        <div className="flex items-center justify-center">
          <div className="glass-nav flex items-center rounded-full max-w-full lg:max-w-4xl xl:max-w-5xl overflow-x-auto no-scrollbar">
            <div className="flex flex-nowrap py-3 px-2 sm:px-3">
              <ul className="flex px-2 flex-nowrap">
                {navLinks.map((link) => (
                  <li key={link.name} className="px-2 sm:px-3">
                    <a 
                      href={link.href} 
                      className="nav-link-glass block px-3 sm:px-4 py-2 text-sm sm:text-base md:text-lg whitespace-nowrap hover:text-blue transition-colors"
                    >
                      {link.name}
                    </a>
                  </li>
